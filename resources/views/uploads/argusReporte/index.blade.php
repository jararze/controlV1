        ...
        <div class="grid gap-5 lg:gap-7.5 xl:w-[38.75rem] mx-auto">
            <div class="card pb-2.5">
                <div class="card-header" id="basic_settings">
                    <h3 class="card-title">
                        Subir archivo de conducción
                    </h3>
                </div>
                <form action="{{ route('uploads.argusReporte.conduccion.post') }}" method="POST" enctype="multipart/form-data" class="card-body grid gap-5" x-data="{ fileName: '' }">
                    @csrf

                    <!-- Tipo de tabla -->
                    <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5">
                        <label class="form-label max-w-56">Tipo de archivo</label>
                        <select name="tipo" class="select select-lg " required>
                            <option value="limite">Límite de Conducción</option>
                            <option value="excesos">Excesos de Velocidad</option>
                        </select>
                    </div>
                ...
                @if (session('warning'))
                    <div class="w-full mb-4 p-4 bg-yellow-500 text-white rounded">
                        {{ session('warning') }}
                    </div>
                @endif
